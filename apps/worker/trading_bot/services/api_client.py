...
class TradingBotApiClient:
    ...
    async def execute_testnet_trade(self, trade_plan_id: int) -> dict:
        async with httpx.AsyncClient(timeout=20.0) as client:
            response = await client.post(f"{self.base_url}/testnet-trading/execute/{trade_plan_id}")
            response.raise_for_status()
            return response.json()

    async def sync_open_testnet_exits(self) -> dict:
        async with httpx.AsyncClient(timeout=20.0) as client:
            response = await client.post(f"{self.base_url}/testnet-trading/sync-open-exits")
            response.raise_for_status()
            return response.json()
