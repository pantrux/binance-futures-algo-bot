# ADRs (Architecture Decision Records)

## Activos

- `ADR-001-stack-base.md`
- `ADR-002-risk-engine.md`
- `ADR-003-outline-as-source-of-truth.md`
- `ADR-004-synology-runtime.md`
- `ADR-005-trade-plan-persistence.md`
- `ADR-006-paper-trading-first.md`
- `ADR-007-seed-and-demo-loop.md`
- `ADR-008-hybrid-worker-market-signals.md`
- `ADR-009-market-ingestion-hardening.md`
- `ADR-010-technical-indicators-base.md`
- `ADR-011-signal-features-base.md`
- `ADR-012-observability-hardening-baseline.md`
- `ADR-013-synology-deployment-smoke-gate.md`
- `ADR-014-synology-preflight-gate.md`
- `ADR-015-synology-release-gate-unificado.md`
- `ADR-016-release-gate-json-summary.md`
- `ADR-017-release-gate-json-verification.md`
- `ADR-018-ops-make-targets.md`
- `ADR-019-operational-signoff-checklist.md`
- `ADR-020-signoff-package-consolidation.md`
- `ADR-021-complete-signoff-workflow.md`
- `ADR-022-phase5-operational-closure.md`
- `ADR-023-artifact-retention-policy.md`
- `ADR-024-operational-observability-alerting.md`
- `ADR-025-resilience-backup-recovery-hardening.md`
- `ADR-026-market-regime-classifier.md`
- `ADR-027-dynamic-risk-sizing-by-regime.md`
- `ADR-028-portfolio-correlation-risk-guards.md`
- `ADR-029-final-decision-gate-and-circuit-breakers.md`
- `ADR-030-router-binance-testnet.md`
- `ADR-031-execution-reconciliation-state-machine.md`
- `ADR-032-paper-vs-testnet-parity-shadow-run.md`
- `ADR-033-production-alerting-and-daily-reporting.md`
- `ADR-034-backtesting-walk-forward.md`
- `ADR-035-transition-checklist-capital-ramp.md`
- `ADR-036-cutover-and-post-cutover-monitoring.md`
- `ADR-037-cutover-drills-and-evidence-package.md`
- `ADR-038-cutover-artifact-templates.md`
- `ADR-039-outline-link-rewrite-strategy.md`
- `ADR-040-shadow-run-gate-auditable.md`

## Archivados

- `archive/ADR-008-market-ingestion-foundation.md` (se movió para eliminar conflicto de numeración con ADR-008 activo)
